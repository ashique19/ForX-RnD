...
The desk proxies `/health`, `/watchlist`, `/assets`, `/board`, `/brief`, `/consensus`, `/ohlcv`, `/refresh`, `/paper`, `/pipeline`, and `/learnings` to port **8000**. Clocks on the desk are **Asia/Dhaka**. `STALE` / `MISSING` stay labeled — a stale row is not a live BUY/SELL. Session (Asia / London / NY) is a separate badge from freshness; a last bar older than ~2× its timeframe is **STALE**, not Live. Paper Buy/Sell/Close writes only the local `PaperBroker` journal. External forecasters live in a registry (`api/consensus_registry.py`): public APIs and feeds (FXEmpire technical JSON, StockTwits tags, ActionForex RSS) plus HTML parsers (Investing.com, DailyForex, FXSSI, IG client sentiment, Mataf pivots, FXStreet). Sources blocked by robots, a bot check, or a dead host (TradingView scanner, Myfxbook, Dukascopy SWFX, DailyFX, FX Blue) are listed as skipped and are not requested. The watchlist may list many pairs, but exactly one is Active. Selecting a row in the Decision watchlist (or a Lab board row) sets that pair Active and queues a forecaster fetch for it alone. A price refresh of the other pairs does not scrape them, and switching Active drops any pair still waiting so the desk does not refresh every symbol at full depth. The selected pair's price refresh also ensures **1h and 1d** OHLCV. Other pairs stay on their own interval. 1d is a real provider download; if the provider cannot fill a missing daily file, it is aggregated from the 1h cache (`resampled_from_1h` — open/high/low/close/volume only, no synthetic prices). A daily horizon that is still missing says `Daily — failed: …` (and `Last OK` in Asia/Dhaka when a previous fetch time is known) instead of a bare MISSING chip. A background job refreshes `data/consensus_cache.json` (gitignored, about 30 minutes) for the Active pair so the brief does not wait on the network. The card shows how many sources are OK (for example 6/14 OK), Buy/Sell/Neutral counts, agreement only when at least two sources published a side, and the span of published ranges. A failed source is never a blank MISSING: the row shows the status, a short reason such as HTTP 403, timeout, empty parse, pair not listed, or rate limited, and the last successful time in Asia/Dhaka when one exists. A direction or price is shown only when that source stated one. **Data refresh** and **Run pipeline** are different. The strip under the nav shows **Last update …s ago**, **Updating in …s**, and **Update now**. Auto-refresh is on by default and calls `POST /refresh` for every watched pair (plus the open chart timeframe) every `data_refresh_seconds`. That value is the server floor `FORX_REFRESH_MIN_S`, default **18s** (rounded up). The call re-reads yfinance into the cache and redraws the board, brief, and chart in place. It does **not** train, backtest, or generate signals, and it never invents prices. **Run pipeline** stays a manual Lab action. The chart **Realtime** switch is the same auto interval. While a refresh is in flight the strip says **Updating…**. A failure says **API unreachable**, **Rate-limited**, or **Refresh failed**, with **Retry**. If the API, desk, or network is unreachable, a reconnect banner names which one and shows **Retrying in N seconds** plus **Reconnect**. Update now inside the limiter window keeps the cached rows and shows Rate-limited until the window opens. The chart and brief reload icons use the same Update now path and do not blank the panels.
...
python -m forex_lab replay --pair EURUSD --interval 1h --start 2015-01-01
```

Historic replay downloads Dukascopy ticks (HistData fallback) into gitignored `data/history/` and walks a `ReplayClock` so each decision only sees bars ≤ the clock. Paper champion / challenger / SMA books are separate from the live journal. Scoreboard CSV, xlsx, and an equity/drawdown chart land in `data/replay/`. Report clocks are Asia/Dhaka. The current-week calendar is not a point-in-time history, so the event-window gate is not applied on replay; bar fills are. The watchlist can hold many pairs; exactly one is Active. The Decision desk **Replay train** button pulls and replays that Active pair only, and a second historic job is refused while one is running. After a replay finishes, the desk shows that scoreboard inline (trades, win rate, expectancy, net P/L, max drawdown, profit factor, and the promotion verdict) for the Active pair, including the last successful run after a refresh. Equity chart and CSV/xlsx/report links stay available. Replay promotion is advisory research: it does not replace the live weekly champion until the weekly retrain gate says so, and it does not overwrite `data/champion`. A failed pull or train keeps a reason on the job (`download`, `decode`, `insufficient_bars`, `train`) and shows that sentence on the button and in the job status — an empty exception still names the error type.
...
**Theme:** the desk ships a **dark dense terminal** look — high-contrast **BUY green / SELL red / HOLD grey**, body type ≥16px, captions ≥14px, near-white primary / light-grey secondary. **Cards** sit on an elevated surface with visible borders (board, expanders, detail drawer, advice/risk). **Nav** is a masthead + workspace row, then **scan board → detail drawer → alerts / awareness / digest**. Drawer tabs are large and clickable. **STALE / MISSING** (amber / muted) outrank the flash so a stale row never reads as a live call. Empty watchlist / calendar / paper states are labeled, not blank. Config is `.streamlit/config.toml` plus custom CSS in `forex_lab/ui/theme.py`. This is presentation only: clocks stay **Asia/Dhaka**, paper `BrokerPort` is unchanged, and no features are removed.

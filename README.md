...
**Theme:** light desk by default (TradingView-like) — **dark text** (`#0f172a`) on a near-white page, white cards, visible borders, body **17px**, captions **15px**, slate muted grey (not washed-out). **BUY green / SELL red / HOLD grey** stay high-contrast. Optional **Light / Dark** toggle sits in the masthead. Config is `.streamlit/config.toml` plus CSS in `forex_lab/ui/theme.py`. Presentation only: clocks stay **Asia/Dhaka**, paper `BrokerPort` is unchanged.

**Navigation:** a prominent top bar **Decision | Calendar | Paper | Lab | Awareness** with the **Asia/Dhaka clock** (date + time, **BD · Asia/Dhaka**) on the right. The clock updates on rerun and when Realtime refreshes — it is not a standalone Decision card. Only one mode’s main content is on screen — this is not a dump of every panel on one scroll. **Decision** (default) is the trader scan: compact alerts, watchlist board, selected-pair detail drawer. **Calendar / Paper / Lab / Awareness** are separate destinations with their own chrome. Helper cards (**Nav / Workspace / lab TF · click a pair for the drawer**, column help, mode intro copy) start **collapsed** behind a ▸/▾ icon; expand state lives in `st.session_state`. Critical **STALE / flip alerts** stay on a visible strip, not inside a collapsed card. **STALE / MISSING** outrank the flash. Empty / STALE states are one message plus **Fetch**. Drawer tabs stay large and clickable. Streamlit’s sticky header is hidden so it does not overlap the mode nav or board.

The **Decision** board is **Pair | Signal | Data● | Last | Next | Actions** (TF / conf / MTF / session sit under those cells). Full columns including spark and spread live in **Lab → Table view**. **BUY** is green, **SELL** red, **HOLD** grey. Click a pair to open a **signal brief**: `{PAIR} · bias · Asia/Dhaka time`, then **Hourly** and **Daily** suggestion lines in the form `Potential buy: now at {last}, stop loss {sl}, target {tp}, duration {hours or 1-2 days}` (HOLD → No trade). SL/TP are ATR barriers on that horizon’s cache — never invented; missing TF shows `need Fetch/Train for {TF}`. **If scenario changes** lists SL-break / MTF conflict / STALE / calendar invalidation. 2–4 sentences of **why**, then a **technical** chart. **Add pair** (pair + timeframe) sits on Decision and writes `config/watchlist.yaml`; **×** on a row removes it. Rows without cached data show **MISSING** plus a **Fetch** path. **Lab** is one **Run pipeline** click (train → backtest → generate signals; optional Also fetch). Individual steps live under **Advanced**.

...
The desk proxies `/health`, `/watchlist`, `/assets`, `/board`, `/brief`, `/consensus`, `/ohlcv`, `/refresh`, `/paper`, and `/pipeline` to port **8000**. Clocks on the desk are **Asia/Dhaka**. `STALE` / `MISSING` stay labeled — a stale row is not a live BUY/SELL. Session (Asia / London / NY) is a separate badge from freshness; a last bar older than ~2× its timeframe is **STALE**, not Live. Paper Buy/Sell/Close writes only the local `PaperBroker` journal. External forecasters (DailyForex, Investing.com, FXStreet) are fetched into `data/consensus_cache.json` (gitignored, about 30 minutes). A direction is shown only when the page states one. **Realtime** polls `POST /refresh` for the selected pair about every `refresh_seconds` (default 60s), then redraws from cache — it is **not** a broker tick stream. `FORX_REFRESH_MIN_S` (default 18s) throttles that network refresh. A manual reload inside the window shows a muted “updated from cache” countdown; automatic polls that are throttled stay quiet and do not raise a red error. The reload button does not blank the brief or the chart.

**Chart indicators.** `GET /ohlcv/{pair}` adds `digits` (5 dp FX, 3 dp JPY, 2 dp gold/silver — the same scale as the desk) and an `indicators` object. Every series has the same length as `bars`. Warm-up slots are `null`; nothing is filled in. Values are computed on the full cached frame with the lab's causal EMA / RSI / ATR helpers and the native stochastic formula, then sliced to the returned window, so EMA 21 and SMA 200 can use bars before the visible tail without those bars being added to the response. Series: EMA 21, EMA 50, SMA 200, Bollinger Bands (20, 2), RSI (14), MACD (12, 26, 9) line / signal / histogram, Stochastic %K/%D (14, 3, 3), and ATR (14). The desk keeps the light candlestick chart and the volume overlay. RSI and MACD sit in panes under price (on by default) with EMA 21 on the price pane. Compact toggles add EMA 50, SMA 200, Bollinger, Stochastic, and ATR; those start off. Toggles stay put when Realtime or reload brings new candles — the chart is updated in place, so the panel does not collapse.
...
**Theme:** light desk by default (TradingView-like) — **dark text** (`#0f172a`) on a near-white page, white cards, visible borders, body **17px**, captions **15px**, slate muted grey (not washed-out). **BUY green / SELL red / HOLD grey** stay high-contrast. Optional **Light / Dark** toggle sits in the slim top nav (under the clock). Config is `.streamlit/config.toml` plus CSS in `forex_lab/ui/theme.py`. Presentation only: clocks stay **Asia/Dhaka**, paper `BrokerPort` is unchanged.

**Navigation:** a prominent top bar **Decision | Calendar | Paper | Lab | Awareness** with the **Asia/Dhaka clock** (date + time, **BD · Asia/Dhaka**) on the right. The clock updates on rerun and when Realtime refreshes — it is not a standalone Decision card. There is **no** “FX SIGNAL SCREEN” hero card and **no** disclaimer expander on the desk; research caveats stay in this README and on board captions. Only one mode’s main content is on screen — this is not a dump of every panel on one scroll. **Decision** (default) is the trader scan: compact alerts, watchlist board, selected-pair detail drawer. **Calendar / Paper / Lab / Awareness** are separate destinations with their own chrome. Helper cards (**Nav / Workspace / lab TF · click a pair for the drawer**, column help, mode intro copy) start **collapsed** behind a ▸/▾ icon; expand state lives in `st.session_state`. Critical **STALE / flip alerts** stay on a visible strip, not inside a collapsed card. **STALE / MISSING** outrank the flash. Empty / STALE states are one message plus **Fetch**. Drawer tabs stay large and clickable. Streamlit’s sticky header is hidden so it does not overlap the mode nav or board.

The **Decision** board is **Pair | Signal | Data● | Last | Next | Actions** (TF / conf / MTF / session sit under those cells). Full columns including spark and spread live in **Lab → Table view**. **BUY** is green, **SELL** red, **HOLD** grey. Click a pair to open a **signal brief** in a DailyForex-like reading column: bold `{PAIR} Forex Signal: {bias}` headline, a short byline, then **Bearish view** and **Bullish view** as stacked headings with Hourly/Daily bullets (`Potential buy/sell: now at {last}, stop loss {sl}, target {tp}` plus **Timeline**). The opposite view reuses the same ATR pair with SL/TP swapped — never invented levels. **Technical chart** sits in that column (Realtime + ↻ on the toolbar), then headed prose (**Why this label**, **If scenario changes**, **Technical notes**). HOLD → No trade. Missing TF → `need Fetch/Train for {TF}`. **Add pair** writes `config/watchlist.yaml`; **×** removes a row. **Lab** is one **Run pipeline** click (train → backtest → generate signals; optional Also fetch).

**Chart** (in the brief): TradingView-like Plotly of **cached** yfinance OHLCV — pair + timeframe chips (`1m` / `15m` / `1h` / `4h` / `1d`, disabled when that CSV is missing), OHLC header (O H L C, change %, volume), green/red candles on a **light** background with dark axes, color-matched volume, EMA(50) plus optional EMA(200) and RSI(14) from the same causal helpers as the model. Directly above the chart: a **Realtime** switch (cache auto-refresh, not broker live) and a **↻** reload button for one-shot update. Not a broker live feed. **STALE / MISSING** block the brief and the candles.

**Modes** — Decision (scan + brief) | Calendar | Paper | Lab | Awareness. One destination at a time. Streamlit’s sticky header is hidden so it does not overlap the mode nav or board.
...
**Realtime** (default **60s**, minimum 60s): the **Realtime** switch sits **above the chart** in the pair drawer. It rebuilds signals from the **local** cache each tick. yfinance is called only when a bar is due or the cache is approaching stale, **one pair per tick**, with exponential backoff after errors or 429-like responses. Yahoo’s download endpoint is unofficial and has no SLA — 60–120s is the practical band; do not set this like a broker stream. A **rate limited — backing off until …** banner appears if throttled. Realtime off: **↻** on the chart toolbar is the one-shot refresh. Not a live broker feed.

**Data validity:** each row’s **Data●** is `OK` / `CLOSED` / `STALE` / `MISSING` / `ERROR`. In a liquid session, a last candle older than ~2× the timeframe is **STALE** and the flash is **—** plus “data stale — refresh required” (the last model class is kept as a note, not as a live call). Weekends / Friday after ~21:00 UTC show **CLOSED** with last bar time — not a false STALE alarm. Last bar, last fetch, and last signal times live in the detail drawer; the board shows **board last refreshed at …**. Clocks on the desk are **Asia/Dhaka** (`ui.timezone`, UTC+6) with a short **BD** tag next to `Asia/Dhaka` in the top nav; stored data stays UTC. Session windows remain UTC.
...
**Sparkline / Risk:** the scan row shows a unicode spark of the last cached closes (empty when STALE/MISSING — no invented prices). Open the pair **Chart** tab for an interactive **Plotly candlestick** of the last `board.candle_bars` cached OHLC bars (light theme, BUY green / SELL red; empty when STALE/MISSING — not a broker chart). The **Risk** panel has ATR SL/TP (same `barrier` config as labels/backtest), R:R, and config spread. Entry is last close as a **proxy** for next-open. Copy states research suggestion only — no lot size, no auto-submit, no live broker order. HOLD or STALE/MISSING → risk n/a. Paper submit uses those SL/TP as defaults when the box is available.

**Awareness** (own mode): a count bar (**N OK · N STALE · N FAIL**) plus the full **Awareness** table for **every source** the desk fetches or observes: watchlist **OHLCV** (price), **news RSS** (headlines), **model file** presence (signals), **calendar** (events), and optional **FRED** (macro; **OFF** when the pack is disabled). Columns: **Source | Observing | Cadence | Last OK | Status**. Cadence is the realtime interval vs **manual** / **daily**. **Last OK** is **Asia/Dhaka**. Status is **OK / STALE / FAIL** (plus **MISSING / CLOSED / OFF**) with a short error — **STALE and FAIL never look OK**. Daily digest lives on this page. CLOSED (weekend) is not a panic. Paper `BrokerPort` is unchanged.
...
Fetch / Train / Backtest / Generate signals and the **champion/challenger retrain gate** live in **Lab** (top nav), with walk-forward CSV/metrics/equity/logs on that same page. Open Lab when you need data or a model, not to read the Decision board.
...
forex_lab/
  ...
  ui/            # Streamlit helpers (watch board, awareness panel, alerts, workspace presets, light desk theme; no live trading)
config/default.yaml
